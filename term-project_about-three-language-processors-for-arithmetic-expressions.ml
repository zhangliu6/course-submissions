...
(* subsidary questions: evaluation order:
...
interpret (Source_program (Plus(Quotient(Literal (an_int 10), Literal (an_int 0)), Quotient(Literal (an_int 100), Literal (an_int 0)))));;
...
let decode_execute bci ds =
 (* decode_execute : byte_code_instruction -> data_stack -> result_of_execution *)
  ...
  | Quo -> (match ds with
            | ( n1 ::  n2 :: ds') -> if n1 = 0 then KO ( "quotient of " ^ string_of_int n2 ^ " over 0")
                                     else OK( (n2 / n1) :: ds')
            | _ -> (KO "stack underflow for Quo"))
  | Rem -> (match ds with
            | ( n1 ::  n2 :: ds') -> if n1 = 0 then KO ( "remainder of " ^ string_of_int n2 ^ " over 0")
...
let compile (Source_program e) =
 (* compile : source_program -> target_program *)
  let rec translate e =
    ...
    match e with
    | Literal n -> [Push n]
    | Plus (e1, e2) -> List.append (List.append (translate e1)  (translate e2))  [Add]
    | Minus (e1, e2) ->  List.append (List.append (translate e1)  (translate e2))  [Sub]
    | Quotient (e1, e2) ->  List.append(List.append (translate e1)  (translate e2))  [Quo]
    | Remainder (e1, e2) ->  List.append(List.append (translate e1)  (translate e2)) [Rem]
...
The compiler translates from left to right. 
...
compile (Source_program (Plus(Quotient(Literal (an_int 10), Literal (an_int 0)), Quotient(Literal (an_int 100), Literal (an_int 0)))));;
...
(* intermediate step:
let compile_v2_init (Source_program e) =
  let rec translate e a =
    match e with
    | Literal n -> 
       [Push n]
    | Plus(e1, e2) ->
       a @ (translate e1 a) @ (translate e2 a) @ [Add]
    | Minus(e1, e2) ->
       a @ (translate e1 a) @ (translate e2 a) @ [Sub]
    | Quotient (e1, e2) ->
       a @ (translate e1 a) @ (translate e2 a) @ [Quo]
    | Remainder (e1, e2) ->
       a @ (translate e1 a) @ (translate e2 a) @ [Rem]
  in Target_program (translate e []);;
...
(* An important realization at this point: the expression is a binary tree, with Literal being the Leaf case; Plus, Minus, Quotient, Remainder being different types of Node cases!

The main challenge is the additional [Add] [Minus] etc.
...
(*
Reference: Mini project about multiplying numbers in a binary tree
...
run (compile (Source_program (Plus(Quotient(Literal (an_int 10), Literal (an_int 0)), Quotient(Literal (an_int 100), Literal (an_int 0))))));;
...
let commutativity_test_one_print () = 
  ...
  let () = match  (interpret p) with Expressible_int n -> Printf.printf "Interpret result %i\n" n
                                      | _ -> ()
    in let () = match ( run(compile p)) with Expressible_int n -> Printf.printf "Run Compile result %i\n" n
                                           | _ -> ()
       in let ()=Printf.printf "\n" in commutativity_test interpret compile run p;;

commutativity_test_one_print;;
...
let compile_v1_fold_right (Source_program e) =
  ...
  let rec translate e =
    ...
    in let plus_case e1 e2 = List.append (List.append e1 e2)  [Add]
       in let minus_case e1 e2 = List.append (List.append e1 e2)  [Sub]
          in let quotient_case e1 e2 = List.append (List.append e1 e2) [Quo]
             in let remainder_case e1 e2 = List.append (List.append e1 e2)  [Rem]
                in fold_right_arithmetic literal_case plus_case minus_case quotient_case remainder_case (Source_program e)
in Target_program (translate e);;
